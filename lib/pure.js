...
function stripReplicaSuffix(name) {
  return String(name || '').replace(/\s*\(replica\s*\d+\)$/i, '');
}

function replicaIdFromName(name) {
  const m = String(name || '').match(/\(replica\s*(\d+)\)$/i);
  return m ? Number(m[1]) : 0;
}

function parsePartitionFromName(name) {
  const s = String(name || '');
  let m = s.match(/^(.*)\s+#(\d+)$/);
  if (m) return { name: m[1], partitionId: Number(m[2]) };
  m = s.match(/^(.*):(\d+)$/);
  if (m) return { name: m[1], partitionId: Number(m[2]) };
  return { name: s, partitionId: null };
}

function parseStatsKey(key) {
  const p = String(key).split(':');
  let bucket, scope, collection, name, partitionId = null;
  if (p.length === 2) {
    bucket = p[0]; scope = '_default'; collection = '_default'; name = p[1];
  } else if (p.length === 4) {
    bucket = p[0]; scope = p[1]; collection = p[2]; name = p[3];
  } else if (p.length === 5 && /^\d+$/.test(p[4])) {
    bucket = p[0]; scope = p[1]; collection = p[2]; name = p[3];
    partitionId = Number(p[4]);
  } else if (p.length === 3 && /^\d+$/.test(p[2])) {
    bucket = p[0]; scope = '_default'; collection = '_default'; name = p[1];
    partitionId = Number(p[2]);
  } else {
    bucket = p[0];
    scope = p.length > 2 ? p[1] : '_default';
    collection = p.length > 3 ? p[2] : '_default';
    name = p[p.length - 1];
  }
  const parsed = parsePartitionFromName(name);
  return {
    bucket, scope, collection,
    name: parsed.name,
    partitionId: partitionId != null ? partitionId : parsed.partitionId
  };
}

function indexFamilyKey(idx) {
  return idx.bucket + ':' + idx.scope + ':' + idx.collection + ':' + stripReplicaSuffix(idx.name);
}

function markPartitionedIndexes(allIndexes) {
  const masterNodes = {};
  (allIndexes || []).forEach(idx => {
    if (replicaIdFromName(idx.name) > 0) return;
    const k = indexFamilyKey(idx);
    if (!masterNodes[k]) masterNodes[k] = new Set();
    masterNodes[k].add(idx.currentNode || idx._nodeName);
  });
  (allIndexes || []).forEach(idx => {
    if (idx.isPartitioned) return;
    const k = indexFamilyKey(idx);
    if ((idx.partitionId != null && idx.partitionId > 0) || (masterNodes[k] && masterNodes[k].size > 1)) {
      idx.isPartitioned = true;
    }
  });
  return allIndexes;
}

function isFullyResident(idx) {
  return idx != null && idx.resident_percent != null && Number(idx.resident_percent) >= 100;
}

function hasResidentMetric(idx) {
  return idx != null && idx.resident_percent != null;
}

function computeNodeMemoryResidency(nodes) {
  const perNode = (nodes || []).map(n => {
    const flat = n.flat || [];
    const known = flat.filter(hasResidentMetric);
    const fully = known.filter(isFullyResident);
    const avgResidentPct = known.length
      ? known.reduce((s, x) => s + Number(x.resident_percent), 0) / known.length
      : null;
    return {
      nodeName: n.nodeName,
      indexCount: flat.length,
      knownCount: known.length,
      fullyResidentCount: fully.length,
      fullyResidentPct: known.length ? (100 * fully.length) / known.length : null,
      avgResidentPct,
      memoryUsed: flat.reduce((s, x) => s + (x.memory_used || 0), 0)
    };
  });
  const clusterKnownCount = perNode.reduce((s, r) => s + r.knownCount, 0);
  const clusterFullyResidentCount = perNode.reduce((s, r) => s + r.fullyResidentCount, 0);
  return {
    perNode,
    clusterFullyResidentCount,
    clusterKnownCount,
    clusterFullyResidentPct: clusterKnownCount
      ? (100 * clusterFullyResidentCount) / clusterKnownCount
      : null,
    avgFullyResidentIndexesPerNode: perNode.length
      ? perNode.reduce((s, r) => s + r.fullyResidentCount, 0) / perNode.length
      : 0
  };
}
...
function buildCreateIndex(idx) {
  ...
  let stmt;
  if (idx.isPrimary) {
    stmt = `CREATE PRIMARY INDEX \`${idx.name}\` ON ${ks}`;
  } else {
    const keys = (idx.keys || []).join(', ');
    stmt = `CREATE INDEX \`${idx.name}\` ON ${ks}(${keys})`;
  }
  if (idx.partition) stmt += ` PARTITION BY ${idx.partition}`;
  if (!idx.isPrimary && idx.condition) stmt += ` WHERE ${idx.condition}`;
  const withParts = [];
  if (idx.replica > 0) withParts.push(`"num_replica":${idx.replica}`);
  if (idx.numPartition) withParts.push(`"num_partition":${idx.numPartition}`);
  if (withParts.length) stmt += ` WITH {${withParts.join(', ')}}`;
  return stmt;
}
...
function dedup(arr) {
  const seen = new Set();
  return arr.filter(x => {
    if (replicaIdFromName(x.name) > 0) return false;
    const part = x.partitionId != null ? String(x.partitionId) : '';
    const k = [x.bucket, x.scope, x.collection, stripReplicaSuffix(x.name), x._nodeName || '', part].join(':');
...
function matchFilter(item, f, _getScanTime) {
  ...
  return (!f.bucket || item.bucket === f.bucket) &&
         ...
         (!f.index || stripReplicaSuffix(item.name) === f.index);
}
...
function parseSystemJSON(js) {
  ...
  d.forEach(r => {
    ...
    flat.push({
      ...
      isPartitioned: !!(idx.partition && String(idx.partition).trim()),
      numPartition: (idx.metadata && (idx.metadata.num_partition || idx.metadata.numPartition)) || null,
...
function parseStatsNodeJSON(js, metric) {
  ...
  for (const [key, s] of Object.entries(d)) {
    ...
    const bucket = parsed.bucket, scope = parsed.scope, collection = parsed.collection, name = parsed.name;
    const ds = s.disk_size || 0, da = s.data_size || 0;
    flat.push({
      fullKey: key, bucket, scope, collection, name,
      partitionId: parsed.partitionId,
      isPartitioned: parsed.partitionId != null && parsed.partitionId > 0,
...
function computeRebalancePlan(opts) {
  ...
  filtered.forEach(node => {
    node.flat.forEach(idx => {
      allIndexes.push({
        ...
        isPartitioned: !!idx.isPartitioned, partitionId: idx.partitionId != null ? idx.partitionId : null,
        _replicaNodes: idx._replicaNodes || [node.nodeName], _replicaTotal: idx._replicaTotal || 1, _replicaIndex: idx._replicaIndex || 1
      });
    });
  });

  markPartitionedIndexes(allIndexes);
  ...
  function pinToCurrentNode(idx) {
    const nodeName = idx.currentNode;
    assignment[nodeName].push({ ...idx, fromNode: idx.currentNode, toNode: idx.currentNode });
    recordAssignment(nodeName, idx);
  }
  function groupIsPartitioned(replicas) {
    return replicas.some(r => r.isPartitioned);
  }
  ...
  if (strategy === 'stratified') {
    ...
    sortedStrata.forEach(([, stratumIndexes]) => {
      ...
      sorted.forEach(([, replicas]) => {
        if (groupIsPartitioned(replicas)) { replicas.forEach(pinToCurrentNode); return; }
        const usedNodes = [];
...
function estParseIndex(sql) {
  ...
  const whereMatch = s.match(/WHERE\s+(.+?)(\s+PARTITION\s+BY\s+|\s+WITH\s+|\s+USING\s+|$)/i);
  const whereClause = whereMatch ? whereMatch[1].trim() : null;
  let partitionBy = '';
  const partIdx = s.search(/\bPARTITION\s+BY\s+/i);
  if (partIdx !== -1) {
    const rest = s.substring(partIdx).replace(/^\s*PARTITION\s+BY\s+/i, '');
    if (/^HASH\s*\(/i.test(rest)) {
      ...
      if (end >= 0) partitionBy = rest.substring(0, end + 1).trim();
    }
  }
  let numPartition = null;
  const withMatchEst = s.match(/\bWITH\s+(\{.*\})\s*;?\s*$/i);
  if (withMatchEst) {
    try {
      const w = JSON.parse(withMatchEst[1]);
      if (w.num_partition != null) numPartition = Number(w.num_partition);
    } catch (e) { /* ignore invalid WITH here; estValidateIndexStr reports it */ }
  }
  ...
  const resolvedFields = keyExpressions.map(expr => {
    ...
    if (metaMatch)
      return { expr, field: 'meta().' + metaMatch[1].toLowerCase(), isArray: false, isMeta: true };
  ...
  return { indexName, keyspace, bucket, scope, collection, keyExpressions, resolvedFields, whereClause, partitionBy, numPartition };
}
...
/**
 * Classify the relationship between a new index and an existing index.
 * Returns { relationship, desc } where relationship is one of:
 *   'exact', 'same-fields', 'replaces', 'covered-by', 'similar', 'none'
 */
function estClassifyRelationship(newFields, newWhere, newBucket, newScope, newCollection,
  ...
  const sameFieldsAnyOrder = newFields.length === existFields.length && newFields.length > 0 &&
    [...newFieldsSet].every(f => existFieldsSet.has(f)) && [...existFieldsSet].every(f => newFieldsSet.has(f));
  const nw = (newWhere || '').replace(/\s+/g, ' ').trim().toLowerCase();
  const ew = (existWhere || '').replace(/\s+/g, ' ').trim().toLowerCase();
  const sameWhere = nw === ew;
  const sameTarget = (newBucket || '').toLowerCase() === (existBucket || '').toLowerCase() &&
    (newScope || '_default').toLowerCase() === (existScope || '_default').toLowerCase() &&
    (newCollection || '_default').toLowerCase() === (existCollection || '_default').toLowerCase();

  const existHasWhere = ew.length > 0;
  const newHasWhere = nw.length > 0;
  const whereMismatch = existHasWhere !== newHasWhere;
  const whereNote = whereMismatch
    ? (existHasWhere ? ' Note: existing index has a WHERE clause (narrower subset of data).' : ' Note: new index has a WHERE clause (narrower subset of data).')
    : '';

  if (sameFieldsAndOrder && sameWhere && sameTarget) {
    return { relationship: 'exact', desc: 'Identical index already exists — same fields, same order, same WHERE clause, same bucket.scope.collection.' };
  } else if (sameFieldsAndOrder && sameTarget && !whereMismatch) {
    return { relationship: 'same-fields', desc: 'Same fields in same order but different WHERE clause.' };
  } else if (sameFieldsAnyOrder && sameWhere && sameTarget) {
    return { relationship: 'same-fields', desc: 'Same fields but in different order. Field order affects query covering and performance.' };
  } else if (sameFieldsAnyOrder && sameTarget && !whereMismatch) {
    return { relationship: 'same-fields', desc: 'Same fields but different order and different WHERE clause.' };
  } else if (newFields.length > 0 && existFields.length > 0 && [...existFieldsSet].every(f => newFieldsSet.has(f)) && newFields.length > existFields.length && sameTarget && !existHasWhere) {
    const extra = newFields.filter(f => !existFieldsSet.has(f));
    return { relationship: 'replaces', desc: 'New index is a superset — covers all fields of existing index plus: ' + extra.join(', ') + '. The existing index could potentially be dropped.' };
  } else if (newFields.length > 0 && existFields.length > 0 && [...newFieldsSet].every(f => existFieldsSet.has(f)) && existFields.length > newFields.length && sameTarget && (sameWhere || (!newHasWhere && existHasWhere))) {
    return { relationship: 'covered-by', desc: 'Existing index already covers all fields of the new index.' + (existHasWhere && !newHasWhere ? '' : ' The new index may be unnecessary.') + whereNote };
  } else if (fieldSimilarity >= 30 && sameTarget) {
    const overlap = newFields.filter(f => existFieldsSet.has(f));
    const onlyNew = newFields.filter(f => !existFieldsSet.has(f));
    const onlyExist = existFields.filter(f => !newFieldsSet.has(f));
    return { relationship: 'similar', desc: 'Field overlap: ' + overlap.join(', ') + (onlyNew.length ? ' | Only in new: ' + onlyNew.join(', ') : '') + (onlyExist.length ? ' | Only in existing: ' + onlyExist.join(', ') : '') + whereNote };
  }
  return { relationship: 'none', desc: '' };
}

// ===== Duplicate Index Finder Helpers =====

function dupPrepIndex(idx) {
  return {
    ...idx,
    _id: idx.fullKey,
    _target: idx.bucket + ':' + idx.scope + ':' + idx.collection,
    _fields: estNormalizeFields(idx.keys),
    _where: (idx.condition || '').replace(/\s+/g, ' ').trim().toLowerCase(),
    _create: buildCreateIndex(idx),
  };
}
...
function dupComparePair(a, b) {
  ...
  const ab = estClassifyRelationship(a._fields, a._where, a.bucket, a.scope, a.collection,
    b._fields, b._where, b.bucket, b.scope, b.collection);
  const ba = estClassifyRelationship(b._fields, b._where, b.bucket, b.scope, b.collection,
    a._fields, a._where, a.bucket, a.scope, a.collection);
...
function dupScanBadge(scanTimeNs) {
  ...
  if (days <= 182) return '91d-6mo';
  if (days <= 365) return '6mo-1y';
  return '1y+';
}
...
module.exports = {
  ...
  stripReplicaSuffix, replicaIdFromName, parsePartitionFromName, parseStatsKey,
  indexFamilyKey, markPartitionedIndexes, isFullyResident, hasResidentMetric,
